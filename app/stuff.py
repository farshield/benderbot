...
import requests
import re
import utils.upsidedown.upsidedown as upsidedown
...
@benderbot_app.register_cmd(r'^!flip')
@benderbot_app.register_cmd(r'^!(coin)?toss')
def cointoss(data):
    coin = "HEADS" if random.randint(1, 100) <= 50 else "TAILS"
    return "<@{}>, the coin landed on: `{}`".format(data['user'], coin)


@benderbot_app.register_cmd(r'^!roll')
def roll(data):
    cmd = data['text'].split()
    if len(cmd) == 2:
        rolls = []
...
@benderbot_app.register_cmd(r'^!tableflip')
def tableflip(data):
    cmd = data['text'].split()
    if len(cmd) == 1:
        return u'(╯°□°）╯︵ ┻━┻'
...
@benderbot_app.register_cmd(r'^!insult')
def insult(data):
    message = ""
    cmd = data['text'].split()
    try:
